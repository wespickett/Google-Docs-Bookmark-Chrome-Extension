...
(function (jQuery, window, document) {
	...
	$(document).ready(function () {
		...
		var gCtx = $('#gview-embed-content').contents(),
			syncImg;

		syncImg = new Image();
		syncImg.src = 'http://wespickett.com/testx?bookmarkPDF=' + docId;
		syncImg.setAttribute("id","bookmarksyncIn");
		syncImg.onload = function () {
			var bookmarked = this.width,
				bookmarkedPage,
				page;

			if (localStorage[docId] > bookmarked) {
				bookmarked = localStorage[docId];
				saveBookmark();
			}
				
			if (bookmarked >= 0) {
				page = $('.thumb-elements', gCtx).children().eq(bookmarked-1);
				bookmarkedImage.prependTo($(page).children().first());
				page.addClass("is-bookmarked");
				$("#bookmarked").css(buttonCSS);
				bookmarkedPage = $("#page-pane", gCtx).children().eq(bookmarked-1);
				$("#content-pane", gCtx).scrollTop(bookmarkedPage[0].offsetTop);
			}
		};

		$(".thumb-element", gCtx).mouseenter(function () {
			if (!$(this).hasClass("is-bookmarked")) {
				$(this).children("#bookmark").remove();
				bookmarkImage.prependTo($(this));
			...
			if (e.srcElement.id === "bookmark") {
				$(".is-bookmarked", gCtx).removeClass("is-bookmarked");
				$("#bookmarked", gCtx).remove();
				bookmarkedImage.prependTo($(this));
				$(this).addClass("is-bookmarked");
				bookmarked = $(this).children(".page-number").text();
				saveBookmark();
				$(this).find("#bookmark").remove();
				$("#bookmarked").css(buttonCSS);
